...
- 自定义固定订阅名称，导入 Clash 时自动命名（例如 RN443）
- 粘贴已有订阅链接更新节点/IP，保留链接；新建及更新后的记录不设置过期时间
- 每份新订阅使用独立访问令牌，服务端只保存令牌哈希；兼容旧 `SUB_ACCESS_TOKEN` 链接
- 支持导出：Raw（Base64）/ Clash（YAML）/ Surge（文本）

## 固定名称与更新 IP

1. 将“固定订阅名称”填为 `RN443`（也可为 RN80 等其他名称）。这与“备注前缀”不同，备注前缀只影响内部节点名。
2. 首次创建时留空“更新已有订阅链接”，填写主节点和优选 IP 后生成。保存完整的 Clash 订阅链接并导入客户端。
3. 下次更换 IP 时，在“更新已有订阅链接”粘贴这条完整链接，填写主节点及新的 IP，再点“更新订阅”。名称不变时仍为 RN443，链接保持不变，在 Clash 刷新即可。
4. 当前页面生成成功后会自动填入这条链接，方便继续更新；刷新或关闭网页后需重新粘贴。要新建另一份订阅，请清空该字段。

旧版订阅首次更新时会升级为独立访问令牌，页面会提示将客户端链接替换一次；后续更新保持完整链接不变。未迁移的旧记录沿用原来的 7 天过期时间，已过期或没有访问令牌的旧记录需重新创建。现有手动名称由客户端保留，新导入时通过 `Content-Disposition` 自动设置名称。

订阅链接同时是读取和更新凭据，请勿公开分享。新建不再按相同输入跨请求去重；只有持有原链接才能更新对应记录。Workers KV 更新存在传播延迟，客户端短时间内仍可能读到旧内容，请稍后刷新。

部署沿用现有 `SUB_STORE` 绑定；`SUB_ACCESS_TOKEN` 仅用于未迁移的旧订阅。不要更换或删除现有 KV。代码回滚不会恢复已迁移记录的旧访问方式，应保留新版以读取带独立令牌的记录。

验证：`npm run check` 覆盖核心转换和 Worker 创建、更新、命名、权限及旧订阅迁移。
