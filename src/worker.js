...
function renderClash(nodes) {
  ...
  return [
    ...
    `mode: rule`,
    `log-level: info`,
    // RN/DMIT exit through a static SOCKS IP that has no IPv6. Phones in VPN mode would otherwise
    // connect to Google over IPv6 and get reset; keep DNS A-only and sniff SNI for IP-literal flows.
    ...(isRackNerd ? [
      `ipv6: false`,
      `dns:`,
      `  enable: true`,
